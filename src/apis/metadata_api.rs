...
pub async fn is_alive(configuration: &configuration::Configuration, ) -> Result<models::HealthStatus, Error<IsAliveError>> {
...
pub async fn is_ready(configuration: &configuration::Configuration, ) -> Result<models::IsReady200Response, Error<IsReadyError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::IsReady200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::IsReady200Response`")))),
        }
    } else {
        let content = resp.text().await?;
